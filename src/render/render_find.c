...
void render_find_bar(Editor *e) {
    ...
    SDL_FRect borders[4] = {{(float)x, (float)y, (float)w, 1},
                            {(float)x, (float)(y + h - 1), (float)w, 1},
                            {(float)x, (float)y, 1, (float)h},
                            {(float)(x + w - 1), (float)y, 1, (float)h}};
    for (int i = 0; i < 4; i++)
        SDL_RenderFillRect(e->renderer, &borders[i]);

    /* helper: dibuja un campo de texto */
#define DRAW_FIELD(fx, fy, fw, focused)                                                            \
    do {                                                                                           \
        set_color(e->renderer, (focused) ? 0x2A : 0x25, (focused) ? 0x2E : 0x29,                   \
                  (focused) ? 0x38 : 0x31, 255);                                                   \
        SDL_FRect _bg = {(float)(fx), (float)(fy), (float)(fw), (float)field_h};                   \
        SDL_RenderFillRect(e->renderer, &_bg);                                                     \
        set_color(e->renderer, (focused) ? 0x52 : 0x3A, (focused) ? 0x8B : 0x3F,                   \
                  (focused) ? 0xD4 : 0x4A, 255);                                                   \
        SDL_FRect _b[4] = {{(float)(fx), (float)(fy), (float)(fw), 1},                             \
                           {(float)(fx), (float)((fy) + field_h - 1), (float)(fw), 1},             \
                           {(float)(fx), (float)(fy), 1, (float)field_h},                          \
                           {(float)((fx) + (fw) - 1), (float)(fy), 1, (float)field_h}};            \
        for (int _i = 0; _i < 4; _i++)                                                             \
            SDL_RenderFillRect(e->renderer, &_b[_i]);                                              \
    } while (0)
    ...
    char txt1[512];
    snprintf(txt1, sizeof(txt1), "%s%s", e->find.query,
    ...
    if (e->find.result_line >= 0 && e->find.match_count > 0) {
        ...
        e->find.prev_btn_y = row1_y;
        e->find.prev_btn_w = arrow_w;
        e->find.prev_btn_h = field_h;
        e->find.next_btn_x = next_x;
        e->find.next_btn_y = row1_y;
        e->find.next_btn_w = arrow_w;
        e->find.next_btn_h = field_h;
        ...
        set_color(e->renderer, 0x3A, 0x3F, 0x4A, 255);
        SDL_FRect pb[4] = {{(float)prev_x, (float)row1_y, (float)arrow_w, 1},
        ...
        for (int i = 0; i < 4; i++)
            SDL_RenderFillRect(e->renderer, &pb[i]);
        {
            int tw = 0, th = 0;
            TTF_GetStringSize(e->font, "↑", 0, &tw, &th);
            draw_text(e, "↑", prev_x + (arrow_w - tw) / 2, row1_y + (field_h - FONT_SIZE) / 2, 180,
                      200, 230);
        }

        /* Contador X/N */
        draw_text(e, ctr_lbl, prev_x + arrow_w + 4, row1_y + (field_h - FONT_SIZE) / 2, 97, 175,
                  239);
        ...
        set_color(e->renderer, 0x3A, 0x3F, 0x4A, 255);
        SDL_FRect nb[4] = {{(float)next_x, (float)row1_y, (float)arrow_w, 1},
        ...
        for (int i = 0; i < 4; i++)
            SDL_RenderFillRect(e->renderer, &nb[i]);
        {
            int tw = 0, th = 0;
            TTF_GetStringSize(e->font, "↓", 0, &tw, &th);
            draw_text(e, "↓", next_x + (arrow_w - tw) / 2, row1_y + (field_h - FONT_SIZE) / 2, 180,
                      200, 230);
        }
    } else if (e->find.query_len > 0 && e->find.match_count == 0) {
        draw_text(e, "Sin resultados", field_x + field_w / 2 - 30,
                  row1_y + (field_h - FONT_SIZE) / 2, 200, 80, 80);
    }
    ...
    char txt2[512];
    snprintf(txt2, sizeof(txt2), "%s%s", e->find.replace,
    ...
    SDL_FRect btnb[4] = {{(float)btn_x, (float)row2_y, (float)btn_w, 1},
    ...
    for (int i = 0; i < 4; i++)
        SDL_RenderFillRect(e->renderer, &btnb[i]);
