...
void hashmap_clear(HashMap *m) {
    if (m->state) memset(m->state, 0, m->cap);
    m->count = 0;
}
...
static int hm_grow(HashMap *m, size_t newcap) {
    ...
    nm.cap = 0;
    nm.count = 0;
    if (!hm_alloc(&nm, newcap)) return 0;

    for (size_t i = 0; i < m->cap; i++)
        if (m->state[i]) hm_place(&nm, m->hashes[i], hm_key(m, i), hm_val(m, i));
...
static size_t hm_find(const HashMap *m, const void *key, size_t h) {
    ...
    size_t i = h & mask;
    while (m->state[i]) {
        if (m->hashes[i] == h && m->eq(hm_key(m, i), key, m->key_size)) return i;
        i = (i + 1) & mask;
    }
    return HM_NOT_FOUND;
}
