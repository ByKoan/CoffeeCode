...
int vec_insert(Vec *v, size_t idx, const void *item) {
    ...
    memmove(base + (idx + 1) * v->elem, base + idx * v->elem, (v->len - idx) * v->elem);
...
int vec_remove(Vec *v, size_t idx) {
    if (idx >= v->len) return 0;
    char *base = (char *)v->data;
    memmove(base + idx * v->elem, base + (idx + 1) * v->elem, (v->len - idx - 1) * v->elem);
    v->len--;
    return 1;
}

int vec_remove_range(Vec *v, size_t from, size_t to) {
    if (from > to || to > v->len) return 0;
    if (from == to) return 1;

    char *base = (char *)v->data;
    memmove(base + from * v->elem, base + to * v->elem, (v->len - to) * v->elem);
    v->len -= (to - from);
    return 1;
}
