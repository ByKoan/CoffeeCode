...
void on_mouse_motion(Editor *e, SDL_Event *ev) {
    ...
    if (e->scrollbar_dragging) {
        int text_height =
            e->win_h - NAVBAR_HEIGHT - TAB_BAR_HEIGHT - STATUS_HEIGHT - SHORTCUT_HEIGHT;
...
void on_mouse_button_down(Editor *e, SDL_Event *ev) {
    ...
    if (my >= NAVBAR_HEIGHT && my < NAVBAR_HEIGHT + TAB_BAR_HEIGHT) {
        ...
        for (int i = 0; i < e->tab_count; i++) {
            ...
            if (mx >= t->tab_x && mx < t->tab_x + t->tab_w) {
                /* botón × cerrar */
                if (mx >= t->close_x && mx < t->close_x + 16 && my >= t->close_y &&
                    my < t->close_y + 16) {
                    ...
                    const char *title =
                        e->tabs[i].filepath[0] ? e->tabs[i].filepath : "CoffeeCode - Sin título";
                    SDL_SetWindowTitle(e->window, title);
                }
                break;
            }
    ...
    if (e->menu_open) {
        ...
        if (item >= 0) {
            ...
            int in_btn =
                (mx >= BTN_FILE_X && mx < BTN_FILE_X + BTN_FILE_W && my >= 0 && my < NAVBAR_HEIGHT);
    ...
    if (my >= NAVBAR_HEIGHT + TAB_BAR_HEIGHT) {
        if (e->menu_open) {
            ...
            int mh = menu_total_h();
            if (!(mx >= BTN_FILE_X && mx < BTN_FILE_X + MENU_WIDTH && my >= menu_y &&
                  my < menu_y + mh)) {
                e->menu_open = 0;
                e->needs_redraw = 1;
            }
