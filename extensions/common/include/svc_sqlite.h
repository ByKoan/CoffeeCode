 ...
 *
 * Define la vtable que una extension con sqlite embebido (p.ej. la extension
 * vesta via libvesta) publica con register_service() para que otras
 * extensiones reusen su base de datos sin enlazar su propia copia de sqlite.
 *
 * Es solo el contrato (el header): la IMPLEMENTACION del proveedor sqlite no es
 * de esta fase y se conectara cuando se integre libvesta.  La API es minima:
 * abrir, ejecutar SQL con callback de filas, y cerrar.
 */
