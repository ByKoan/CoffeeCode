...
C, C++, Rust, Zig, D, Go (`-buildmode=c-shared`), Nim, o cualquier otro lenguaje
capaz de compilar a una biblioteca nativa.
...
puede dejar al sistema de build (en el ejemplo `mi-extension` se usa
`coffee_mi_extension@COFFEE_EXT_DLL_SUFFIX@`, que CMake sustituye por `.dll`/`.so`
segun la plataforma).
